...
#from matplotlib.backends.backend_agg.FigureCanvasAgg import print_png as print_png
from matplotlib.backends.backend_qt5agg import NavigationToolbar2QT as NavigationToolbar
from matplotlib.figure import Figure
from matplotlib.ticker import Formatter, FuncFormatter

from PyQt5.uic import loadUiType
from PyQt5.QtCore import QRegExp, QTimer, QSettings, QDir, Qt, QThread
...
class FlaskThread(QThread):
    ...
    def run(self):
        self.application.run(port=5000, host='0.0.0.0', debug=False)
...
class Measurement:
    ...
    def trim(self, start, stop):
        # print('before: min: ' + str(np.min(self.freq)), 'max: ' + str(np.max(self.freq)), 'size: ' + str(np.size(self.freq)))
        a = self.freq[self.freq >= int(start)]
        b = a[a <= int(stop)]
        self.freq = b
        # print('after: min: ' + str(np.min(self.freq)), 'max: ' + str(np.max(self.freq)), 'size: ' + str(np.size(self.freq)))
        self.start = int(start)
        self.stop = int(stop)
        self.size = int(np.size(self.freq))
...
class FigureTab:
    ...
    def add_cursors(self, axes):
        if self.mode == 'gain_short' or self.mode == 'gain_open':
            ...
            columns = ['Freq., kHz', 'Re(Z), \u03A9', 'Im(Z), \u03A9', '|Z|, \u03A9', r'$\angle$ Z, deg', 'SWR',
                       r'|$\Gamma$|', r'$\angle$ $\Gamma$, deg', 'RL, dB']
    ...
    def plot_smith_grid(self, axes, color):
        ...
        # try to add SWR circle around 50 ohms at SWR = 1.4
        load = 1.0
        axis = np.logspace(-4, np.log10(1.0e3), 200) * load
        z = 5 + 1.0j * axis
        gamma = -load + 0.17 + (z - load) / (z + load)
        axes.plot(gamma.real, gamma.imag, color='green', linewidth=0.4, alpha=0.9)
        axes.plot(gamma.real, -gamma.imag, color='green', linewidth=0.4, alpha=0.9)
    ...
    def plot_smith(self):
        ...
        # print(self.vna.dut.start, self.cursors[0], self.vna.dut.size, self.vna.dut.stop)
        # self.vna.dut.prn()

        # print(self.cursors[0], self.cursors[1], self.cursors[2])

        # self.sweep_start

        start = np.min(self.vna.dut.freq)
        stop = np.max(self.vna.dut.freq)
        size = np.size(self.vna.dut.freq)

        self.span1 = Measurement(start, stop, size)
        self.span2 = Measurement(start, stop, size)
        self.span3 = Measurement(start, stop, size)
        self.span4 = Measurement(start, stop, size)

        self.span1.trim(start, self.cursors[0])
        self.span1.prn()
        self.span2.trim(self.cursors[0], self.cursors[1])
        self.span2.prn()
        self.span3.trim(self.cursors[1], self.cursors[2])
        self.span3.prn()
        self.span4.trim(self.cursors[2], stop)
        self.span4.prn()

        gamma = self.vna.gamma(self.span1.freq)
        self.curve1, = axes1.plot(gamma.real, gamma.imag, color='cyan', linewidth=0.2, alpha=0.7)
        gamma = self.vna.gamma(self.span2.freq)
        self.curve2, = axes1.plot(gamma.real, gamma.imag, color='orange', linewidth=0.8, alpha=1.0)
        gamma = self.vna.gamma(self.span3.freq)
        self.curve3, = axes1.plot(gamma.real, gamma.imag, color='violet', linewidth=0.8, alpha=1.0)
        gamma = self.vna.gamma(self.span4.freq)
        self.curve4, = axes1.plot(gamma.real, gamma.imag, color='green', linewidth=0.8, alpha=1.0)

        #    gamma = self.vna.gamma(self.vna.dut.freq)
        #    self.curve3, = axes1.plot(gamma.real, gamma.imag, color = 'red')
        axes1.axis('equal')
    ...
    def update_smith(self):
        if self.mode == 'smith':
            ...
            # self.span1 = Measurement(self.vna.dut.start, self.vna.dut.stop, self.vna.dut.size)
            # self.span2 = Measurement(self.vna.dut.start, self.vna.dut.stop, self.vna.dut.size)
            # self.span3 = Measurement(self.vna.dut.start, self.vna.dut.stop, self.vna.dut.size)
            # self.span4 = Measurement(self.vna.dut.start, self.vna.dut.stop, self.vna.dut.size)
            ...
            self.curve1.set_color('cyan')
            gamma = self.vna.gamma(self.span2.freq)
            self.curve2.set_xdata(gamma.real)
            self.curve2.set_ydata(gamma.imag)
            self.curve2.set_color('orange')
            gamma = self.vna.gamma(self.span3.freq)
            self.curve3.set_xdata(gamma.real)
            self.curve3.set_ydata(gamma.imag)
            self.curve3.set_color('violet')
            gamma = self.vna.gamma(self.span4.freq)
            self.curve4.set_xdata(gamma.real)
            self.curve4.set_ydata(gamma.imag)
            self.curve4.set_color('green')
            self.canvas.draw()
            ...
            pngImage = io.BytesIO() #"templates/gurka.png"
...
class VNA(QMainWindow, Ui_VNA):
    ...
    def set_enabled(self, enabled):
        widgets = [self.rateValue, self.level1Value, self.level2Value, self.corrValue, self.phase1Value,
                   self.phase2Value, self.startValue, self.stopValue, self.sizeValue, self.openSweep, self.shortSweep,
                   self.loadSweep, self.singleSweep, self.autoSweep]
    ...
    def read_cfg_settings(self, settings):
        self.addrValue.setText(settings.value('addr', '192.168.2.10'))
    ...
    def write_s2p(self, gain):
        ...
        if dialog.exec() == QDialog.Accepted:
            ...
            for i in range(freq.size):
                ...
                freq[i] * 1000, np.absolute(gamma[i]), np.angle(gamma[i], deg=True), np.absolute(gain[i]),
                np.angle(gain[i], deg=True)))
            fh.close()
